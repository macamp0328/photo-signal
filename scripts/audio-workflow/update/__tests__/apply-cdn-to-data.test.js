...
describe('apply-cdn-to-data', () => {
  ...
  it('updateConcertWithCdnAndFileName should use mapped filename', () => {
    ...
    expect(updated.audioFile).toBe('https://audio.example.com/prod/audio/11/ps-mapped-track.opus');
  });
  ...
  it('applyCdnToDataUsingAudioIndex should prefer mapped filenames by concert id', () => {
    ...
    expect(updated.concerts[0].audioFile).toBe(
      'https://audio.example.com/prod/audio/1/ps-one.opus'
    );
    expect(updated.concerts[1].audioFile).toBe(
      'https://audio.example.com/prod/audio/2/ps-two.opus'
    );
  });
