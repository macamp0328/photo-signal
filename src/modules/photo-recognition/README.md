...
Photo hashes are required for recognition to work. Each concert must include a `photoHashes` object with per-algorithm arrays.
...
> `npm run generate-hashes` proxies to `scripts/update-recognition-data.js --paths-mode`, so ad-hoc hashes use the exact same pipeline as full dataset refreshes.
...
npm run generate-hashes -- --algorithms dhash

# Generate pHash values (more robust)
npm run generate-hashes -- --algorithms phash

# Direct invocation with custom folders
...
For the recognition to work, each concert in `data.json` must have a `photoHashes` object:
...
4. Add resulting hash array to the matching `photoHashes` algorithm key
